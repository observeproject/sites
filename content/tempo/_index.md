...
expr: |
  avg by (cluster, namespace) (tempodb_blocklist_length{namespace=~".*", container="backend-scheduler"}) / avg by (cluster, namespace) (tempodb_blocklist_length{namespace=~".*", container="backend-scheduler"} offset 7d) > 1.4
for: 15m
labels:
  severity: critical
{{< /code >}}
